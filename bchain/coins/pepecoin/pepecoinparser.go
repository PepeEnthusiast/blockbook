...
// PepecoinParser handle
type PepecoinParser struct {
	*btc.BitcoinLikeParser
}

// NewPepecoinParser returns new PepecoinParser instance
func NewPepecoinParser(params *chaincfg.Params, c *btc.Configuration) *PepecoinParser {
	return &PepecoinParser{BitcoinLikeParser: btc.NewBitcoinLikeParser(params, c)}
}

// GetChainParams contains network parameters for the main Pepecoin network,
// and the test Pepecoin network
...
func (p *PepecoinParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
